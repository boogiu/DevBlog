...
단, 엔진 내부에 게임 로직이 들어오지 않아야 한다. 엔진은 쿨타임이 무엇인지, 버프가 어떤 규칙으로 종료되는지 알 필요가 없다. ==**시간을 계산하고 관리**하지만, 그 시간을 어떤 의미로 사용할지는 클라이언트의 책임==으로 남겨둔다.
...
> [!check] 핵심 개념 
> **TimeContext는 시간 정책에 대한 정보 묶음이다.** DirectX 기반 엔진을 구성하고 있기 때문에 이름은 Context로 맞춘다.
...
> [!warning] 성능 고려 
> 오브젝트를 순회할 때마다 `TimeManager`에서 Context를 조회하면, 오브젝트 수가 많아졌을 때 불필요한 접근 비용이 생길 수 있다.
> 
...
> [!tip] 핵심 아이디어 
> 모든 시스템이 하나의 `EngineTime`만 사용하도록 강제하는 대신, **시스템마다 참조할 시간 기준을 Context로 분리**한다. 나아가, 각 `TimeContext`를 **다형적으로** 설계해 엔진 시간 기반·오디오 시간 기반·커스텀 시간 기반 등을 동일한 인터페이스로 다룰 수 있게 한다.
...
---
`deltaTime`을 넘기는 것과, 그 시간이 **언제 소비되어야 하는지**를 설계하는 것은 다른 문제다.

처음에는 단순히 시간 값을 정확하게 전달하는 것이 목표였다. 하지만 시스템이 늘어나면서 문제의 본질이 달라졌다. 물리, 오디오, 렌더링, 게임 로직은 각자 다른 시간 기준을 가지고 있었고, 그것들을 하나의 `deltaTime`으로 묶으려 할수록 어긋나는 지점이 생겼다.

결국 도달한 관점은 이렇다. **시간을 먼저 확정하고, 그 시간을 소비하는 순서로 시스템을 배치한다.** `TimeContext`로 시간 기준을 분리한 것도, 업데이트 단계를 나눈 것도, FMOD 콜백을 큐로 받아 메인 Tick에서 처리한 것도 — 모두 같은 원칙에서 나온 결론이었다.

시간 관리는 값의 문제가 아니라 **흐름의 설계** 문제다.